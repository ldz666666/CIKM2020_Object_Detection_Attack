...
def infer_yolo(img_path0, img_path1, yolo_model):
    ...
    img1 = resize_small(img1)
        ...
        # --------------------BOX score
    boxes0 = do_detect(yolo_model, img0, 0.5, 0.4, True)
    boxes1 = do_detect(yolo_model, img1, 0.5, 0.4, True)
...
def count_connected_domin_score_single(img_path0,img_path1,max_total_area_rate=0.02, max_patch_number=10):
    ...
    if patch_number > max_patch_number:
        print(img_path1,'patch_number exceed')
        return 0
    if patch_number == 0:
        print(img_path1,'zero patch') 
        return 0
    if total_area_rate > max_total_area_rate:
        print(img_path1,'area exceed')
        return 0
    
    return connected_domin_score
